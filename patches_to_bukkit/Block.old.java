  ...
  implements net.minecraft.src.Block, ITextureProvider
{
  ...
  public void dropNaturally(World world, int i, int j, int k, int l, float f, int i1) {
    if (!world.isStatic) {
      ...
      ArrayList items = getBlockDropped(world, i, j, k, l, i1);
  ...
  public void a(World world, EntityHuman entityhuman, int i, int j, int k, int l) {
    ...
    if ((h()) && (!hasTileEntity(l)) && (EnchantmentManager.hasSilkTouchEnchantment(entityhuman.inventory))) {
      ItemStack itemstack = a_(l);
  ...
  public boolean isBlockSolidOnSide(World world, int x, int y, int z, int side)
  {
    ...
    if ((this instanceof BlockStairs))
    {
      ...
      return ((meta & 0x3) + side == 5) || ((side == 1) && (flipped));
    }
  ...
  {
    if ((this.id == NETHERRACK.id) && (face == 0))
    {
      return true;
    }
    if (((world.worldProvider instanceof WorldProviderTheEnd)) && (this.id == BEDROCK.id) && (face == 0))
    {
      return true;
    }
    return false;
