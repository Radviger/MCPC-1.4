    ...
    public NetworkModHandler(ModContainer var1, Class var2, ASMDataTable var3)
    {
        ...
        if (this.mod != null)
        {
            ...
            if (var5 != null)
            {
                try
                {
                    Method var14 = var2.getDeclaredMethod(var5, new Class[] {String.class});

                    if (var14.isAnnotationPresent(NetworkMod$VersionCheckHandler.class))
                    {
                        this.checkHandler = var14;
                    }
                }
                catch (Exception var12)
                {
                    FMLLog.log(Level.WARNING, var12, "The declared version check handler method %s on network mod id %s is not accessible", new Object[] {var5, var1.getModId()});
                }
            }

            if (this.checkHandler == null)
            {
                String var13 = this.mod.versionBounds();

                if (!Strings.isNullOrEmpty(var13))
                {
                    try
                    {
                        this.acceptableRange = VersionRange.createFromVersionSpec(var13);
                    }
                    catch (InvalidVersionSpecificationException var11)
                    {
                        FMLLog.log(Level.WARNING, var11, "Invalid bounded range %s specified for network mod id %s", new Object[] {var13, var1.getModId()});
                    }
                }
            }

            FMLLog.finest("Testing mod %s to verify it accepts its own version in a remote connection", new Object[] {var1.getModId()});
            boolean var16 = this.acceptVersion(var1.getVersion());
            ...
            {
                IConnectionHandler var15;

                try
                {
                    var15 = (IConnectionHandler)this.mod.connectionHandler().newInstance();
                }
                ...
                NetworkRegistry.instance().registerConnectionHandler(var15);
            }
        ...
        if (var4 == null || !var4.isClient() || FMLCommonHandler.instance().getSide().isClient())
        {
            if (var2 != this.getPacketHandlerDefaultValue())
            {
                ...
                else
                {
                    ...
                    {
                        FMLLog.log(Level.SEVERE, var10, "Unable to create a packet handler instance %s for mod %s", new Object[] {var2.getName(), var1.getModId()});
                        throw new FMLNetworkException(var10);
                    }
