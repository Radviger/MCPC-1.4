...
public class JarDiscoverer implements ITypeDiscoverer
{
    public List discover(ModCandidate var1, ASMDataTable var2)
    {
        ...
        try
        {
            ...
            while (var7.hasNext())
            {
                ...
                if (var9.matches())
                {
                    ...
                    {
                        FMLLog.log(Level.SEVERE, var21, "There was a problem reading the entry %s in the jar %s - probably a corrupt zip", new Object[] {var8.getName(), var1.getModContainer().getPath()});
                        var4.close();
                        throw var21;
                    }
        ...
        {
            FMLLog.log(Level.WARNING, var22, "Zip file %s failed to read properly, it will be ignored", new Object[] {var1.getModContainer().getName()});
        }
        finally
        {
            if (var4 != null)
